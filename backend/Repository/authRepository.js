...
export const saveLoginAttempt = async (
  ...
  try {
    ...
    throw new Error(
      `Error en saveLoginAttempt: al guardar el intento de inicio de sesión: ${error.message}`
    );
  }
};
...
export const getLoginAttempts = async (username, minutes = 15) => {
  try {
    ...
    const attempts = await loginAttemptSchema
      .find({
        username,
    ...
    throw new Error(
      `Error en getLoginAttempts: al obtener los intentos de inicio de sesión: ${error.message}`
    );
  }
};
